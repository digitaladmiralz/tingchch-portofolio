...
const CommandPalette = () => {
  ...
  const visiblePages = MENU_ITEMS.filter((item) => item.isShow !== false);

  const menuOptions: MenuOptionProps[] = [
    {
      title: 'PAGES',
      children: visiblePages.map((menu) => ({
        ...menu,
        closeOnSelect: true,
      })),
    },
    {
      title: 'SOCIALS',
      children: SOCIAL_MEDIA?.filter((m) => m.isShow !== false).map((menu) => ({
        ...menu,
        closeOnSelect: true,
      })),
    },
    {
      ...
      children: EXTERNAL_LINKS?.filter((m) => m.isShow !== false).map(
        (menu) => ({
          ...menu,
          closeOnSelect: true,
        }),
  ...
  const handleFindGoogle = () => {
    const url =
      ...
      encodeURIComponent(queryDebounce) +
      '&ref=tingchch-portfolio';
